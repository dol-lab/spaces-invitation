...
jQuery(function($) {
	...
	Array.from(document.querySelectorAll('input[name=invitation-status][checked]')).forEach(function(input){
	    input.checked = true;
	});
    };
	...
	document.querySelector('.invitation-edit-link').addEventListener('click', function(){
	    ...
	    button.addEventListener('click', function(){
                ...
                send(INVITATION_ADMIN_URL.url, 'invitation_update_token', INVITATION_NONCES.invitation_update_token, {token: value}).then(function(response){
                    node.remove();
	...
	var self = $(this);
	send(INVITATION_ADMIN_URL.url, 'change_invitation_option', INVITATION_NONCES.invitation_link, {option: this.value}).success(function(response){
    ...
    document.addEventListener("spacePrivacyChanged", function (event) {
        send(INVITATION_ADMIN_URL.url, 'get_disabled_options', INVITATION_NONCES.get_disabled_options, {}).success(function(response){
